...
CHANNELS.each_with_index do |ch, idx|
  # 채널 생성
  channel = Channel.find_or_create_by!(youtube_channel_id: "UC_factcheckr_seed_#{idx + 1}") do |c|
    c.name = ch[:name]
...
# ---------- 5. B2B 테스트 데이터 ----------
b2b_user = User.find_or_create_by!(email: "biz@factcheckr.com") do |u|
  u.name = "FactCheckr 기업고객"
  u.user_type = :b2b
  u.password = "seed_password_not_used"
  u.is_active = true
end
...
puts "  B2B: biz@factcheckr.com + 리포트 2개"
...
AdminSetting.find_or_create_by!(key: "resend_from_email") { |s| s.value = "FactCheckr <onboarding@resend.dev>" }
puts "  AdminSetting: #{AdminSetting.count}개"
puts ""
puts "  [관리자 로그인] /admin/login"
puts "    이메일: #{ENV.fetch('ADMIN_EMAIL', 'blek.park@gmail.com')}"
puts "    비밀번호: #{ENV.fetch('ADMIN_PASSWORD', 'factcheckr-admin-2026')}"
